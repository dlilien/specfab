...
fh = Dataset('solutions/ELMER_DRX_%s.nc'%(exprref), mode='r')
...
#        eigenValues = eigenValues[idx,:]
    ...
    #
    axeigvals.plot(steps,eigvals[:,0],'--m',label='$a_{1}$ (tens.)',lw=lw+0.5)
    axeigvals.plot(steps,eigvals[:,1],'--y',label='$a_{2}$ (tens.)',lw=lw+1.0)
    axeigvals.plot(steps,eigvals[:,2],'--c',label='$a_{3}$ (tens.)',lw=lw)
    #
    ...
    # Save figure
    #----------------------
    fout = 'solutions/ELMER_DRX_%s__%i.png'%(exprref, tt+1)
    print('Saving %s'%(fout))
