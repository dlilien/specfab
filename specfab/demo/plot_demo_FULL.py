...
fh = Dataset('solutions/FULL_%s.nc'%(exprref), mode='r')
...
#        eigenValues = eigenValues[idx,:]
    ...
    #
    axeigvals.plot(times,eigvals[:,0],'--r',label='$a_{1}$ (tens.)',lw=lw+0.5)
    axeigvals.plot(times,eigvals[:,1],'--g',label='$a_{2}$ (tens.)',lw=lw+1.0)
    axeigvals.plot(times,eigvals[:,2],'--b',label='$a_{3}$ (tens.)',lw=lw)
    #
    axeigvals.plot(times,eigvals_true[:,0],'-r',label='$a_{1}$ (spec.)',lw=lwtrue+0.5)
    axeigvals.plot(times,eigvals_true[:,1],'-g',label='$a_{2}$ (spec.)',lw=lwtrue+1.0)
    axeigvals.plot(times,eigvals_true[:,2],'-b',label='$a_{3}$ (spec.)',lw=lwtrue)
    
    axeigvals.plot([0,1],[-tt,-tt],':k',lw=lw) 
    axeigvals.set_ylim([0,1])
    axeigvals.set_xlim([0, (Nt - 1) * dt])
    axeigvals.set_xlabel('Time (yr)')
    ...
    # Save figure
    #----------------------
    fout = 'solutions/FULL_%s__%i.png'%(exprref, tt+1)
    print('Saving %s'%(fout))
